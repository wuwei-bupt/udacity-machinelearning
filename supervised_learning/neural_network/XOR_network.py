...
perceptron_input_1 = Perceptron(weights=np.array([1, 0]))
perceptron_input_2 = Perceptron(weights=np.array([0, 1]))
perceptron_input_3 = Perceptron(weights=np.array([1, 1]), threshold=1.5)
perceptron_output = Perceptron(weights=np.array([1, 1, -2]), threshold=0.5)
Network = [
...
def EvalNetwork(inputValues, Network):
    ...
    for layer in Network:
        layer_outputs = []
        for node in layer:
            node_output = node.activate(layer_inputs)
